...
\end{array}
\end{equation}
We use this mapping for mapping values, when we need the default classical definition of the addition.
...
We observe the identity elements are identical for the fields $\BF_2, \BZ, \BQ, \BR, \BC$ and the unit vectors are identical to the unit vectors of the vector spaces $\BF_2^n, \BZ^n, \BQ^n, \BR^n, \BC^n$, all over a field $K$.
...
In coding theory \cite{Hamming1950, wikipedia_en_2025_HammingWeight} the {\em Hamming weight} (number of 1-entries) 
of~ $x \in \{0, 1\}^n$ is defined as:
\begin{equation}
w_H(x) = | \{i \in \{1, \dots, n\} : x_i = 1\}|,
\end{equation}
and the associated {\em Hamming distance} is:
\begin{equation}
d_H(x, y) = w_H(x - y), \text{ over } \BF_2^n.
\end{equation}

If we first apply operation $| \  |$ we map a vector from $\BF_2^n \rightarrow \BR^n$i, by embedding $\BF_2^n = \{0, 1\}^n \subset \BR^n$. On vectors in $\BR^n$ norms are defined and we get:
\begin{equation}
...
norm~\cite{wikipedia_en_2025_NormMathematics}. In some programming languages like C the function is named popcount(). In Go it is the function  ~\texttt{OnesCount(uint x) uint} in package~\texttt{math/bits}.
