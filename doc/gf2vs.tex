...
\begin{equation}
\BZ_2 = \BZ/\BZ2 = \{0, 1\} \subset \BN \subset \BZ \subset \BR
\end{equation}
the subset $\BZ_2$ of $\BZ$, which is a subset of $\BR$. This set is equal to $\BZ/\BZ2$, the cyclic group of order~2. This set holds the values of a bit in computer science.
...
We define the set of all bit vectors of size $n$ see \cite[ch 2.4.1]{Fischer2020}:
\begin{equation}
...
We define the mapping $\varphi$ from $\BN$ to $\BF_2^n$. 
For mapping of an integer $a \in \BN$ to a bit vector $x \in \BF_2^n$ we need to compute
the length $n$ of $x$ with equation~(\ref{size}).
\begin{equation}
	n = \lfloor \log_{2}(a) \rfloor + 1.
	\label{size}
\end{equation}
If we need to map multiple integers $a_i$ to the same vector space we compute $n$ from the maximum of the $a_i$
\begin{equation}
	n = \lfloor \log_{2}(\max(a_i)) \rfloor + 1.
\end{equation}
Equation~(\ref{vector}) defines the mapping for $a \in \BN, a \ge 0$.
\begin{equation}
\begin{array}{rr}
...
In the programming language Go these mapping are used when parsing integer strings and formatting integers.
Internally integers are stored as bit vectors.

\subsection{Operations on vectors}

We define bitwise operations on the bit vectors $x, y, z$ see \cite[ch. 2.4.1]{Fischer2020} and \cite[(1), (2), (3)]{knuth:taocp_v4_f1}.
\begin{equation}
\left.
\begin{array}{rrcrl}
...
\end{description}
Please note the norm of the unit vectors $\| e_i \| = 1, \quad \forall i \in \{1, \dots n \}$.
